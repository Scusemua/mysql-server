...
int main(int argc, char** argv)
{
    ...
    Ndb* myNdb= new Ndb(cluster_connection,
                "hop_bram_vm");  // Object representing the database

    if (myNdb->init() == -1) APIERROR(myNdb->getNdbError());

    const char *eventName= "ack_table_watch0";
    const char *eventTableName= "write_acks_deployment0";
    ...
    while (j < timeout) {
        ...
        if (myNdb->dropEventOperation(the_op)) 
            APIERROR(myNdb->getNdbError());
        the_op = 0;
        j++;
    }
